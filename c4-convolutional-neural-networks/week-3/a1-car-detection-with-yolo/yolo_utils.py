...
def yolo_head(feats, anchors, num_classes):
    ...
    # feats = Reshape((conv_dims[0], conv_dims[1], num_anchors, num_classes + 5))(feats)
...
def draw_boxes(image, boxes, box_classes, class_names, scores=None):
    ...
    font = ImageFont.load_default(size=np.floor(3.5e-2 * image.size[1] + 0.5).astype('int32'))
    ...
    for i, c in list(enumerate(box_classes)):
        ...
        if top - label_size >= 0:
            text_origin = np.array([left, top - font.size])
        else:
        ...
        draw.rectangle(
            [tuple(text_origin), tuple((text_origin[0] + label_size, text_origin[1] + font.size))],
            fill=colors[c])
