...
def gram_matrix_test(target):
    ...
    assert np.allclose(GA[0,:], [ 63.1888, -26.721277, -7.7320204]), "Wrong values."
...
def compute_layer_style_cost_test(target):
    ...
    assert np.isclose(J_style_layer_SG, 14.017804), "Wrong value."
...
def train_step_test(target, generated_image):
    ...
    assert np.isclose(J1, 24087.941, rtol=0.05), f"Unexpected cost for epoch 0: {J1} != {24087.941}"

    J2 = target(generated_image)
    print(J2)
    assert np.isclose(J2, 16669.113, rtol=0.05), f"Unexpected cost for epoch 1: {J2} != {16669.113}"
